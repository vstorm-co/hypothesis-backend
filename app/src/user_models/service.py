...
async def create_user_model_in_db(user_model_data: UserModelCreateInput) -> Record:
    ...
    insert_query = insert(UserModel).values(**user_model_data.model_dump())

    return await database.execute(insert_query)


async def update_user_model_in_db(uuid: str, user_id: int, user_model_data: UserModelUpdateInput) -> Record:
    # Encrypt the api_key
    ...
    update_query = (
        update(UserModel)
        .where(and_(UserModel.uuid == uuid, UserModel.user == user_id))
...
async def change_user_model_active_status(uuid: str, user_id: int) -> Record:
    current_user_model_db = await get_user_model_by_uuid(uuid, user_id)
...
async def delete_user_model_in_db(uuid: str, user_id: int) -> Record:
    delete_query = delete(UserModel).where(and_(UserModel.uuid == uuid, UserModel.user == user_id))
